...
void Graph::remove(Node& rNode)
{
	...
	//for (auto e : rNode.getInEdges())
	//{
	//	m_edges.remove(e);
	//	// delete pE;
	//}
	//for (auto e : rNode.getOutEdges())
	//{
	//	m_edges.remove(e);
	//	// delete e;
	//}
...
void Graph::findShortestPathDijkstra(std::deque<Edge*>& rPath, const Node& rSrcNode, const Node& rDstNode)
{
	...
	{
		Node *u = Q.front();
		for (auto n : Q) // Liste durchgehen un den Knoten mit der kleinsten Distanz zu finden
	  ...
	  //            dist[v] ← alt
	  //            prev[v] ← u
		list<Edge*> &neighnours = u->getOutEdges();
		for (auto n : u->getOutEdges())
		{
			Node* v = &(n->getDstNode());
			double alt = nodeEntry[u].m_distance + n->getWeight();
